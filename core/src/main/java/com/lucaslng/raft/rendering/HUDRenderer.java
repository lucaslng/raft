...
class HUDRenderer implements Disposable {
	...
	// ── Style cache ───────────────────────────────────────────────────────────
	private final LabelStyle hintGreen, hintRed, hintYellow, mainStyle, titleStyle;
	...
	protected HUDRenderer(Assets assets, EventBus events, BuildingRegistry buildingRegistry) {
		...
		BitmapFont bigFont = assets.get("mainBig.ttf", BitmapFont.class);
		mainStyle = new LabelStyle(font, Color.WHITE);
		titleStyle = new LabelStyle(bigFont, Color.WHITE);
		...
		fpsLabel.addAction(new com.badlogic.gdx.scenes.scene2d.Action() {
			@Override
	...
	protected void render(World world, float delta) {
		...
		if (held instanceof Hammer && world.getPlacementGhost().isVisible()) {
			...
			if (tile == null) {
				...
				hintLabel.setStyle(hintYellow);
				hintLabel.setText(tile.getBuilding().getName() + "  [RMB] Open");
			} else {
	 ...
	 */
	protected com.badlogic.gdx.scenes.scene2d.Stage getStage() {
		return stage;
	}
	...
	private Window buildWaterFilterPanel(WaterFilter filter) {
		...
		win.add(new Label("Drip interval: " + WaterFilter.DRIP_INTERVAL + "s", mainStyle)).left().row();
		win.add(new Label("Thirst / drip: +" + (int) (WaterFilter.THIRST_RESTORE * 100) + "%", mainStyle)).left().row();
		win.add(new Label("Status: Running", mainStyle)).left().row();
	...
	private Window buildSailPanel(SailBuilding sail) {
		...
		win.add(new Label("Wind: " + formatBearing(windDeg), mainStyle))
				.left().colspan(4).row();

		// ── Current heading label (updated by slider) ──────────────────────
		Label headingLabel = new Label("Heading: " + formatBearing(sail.getSteerAngleDeg()), mainStyle);
	...
	private Window buildGenericPanel(Building building) {
		...
		win.setMovable(true);
		win.defaults().pad(6f);
		win.add(new Label(building.getName(), mainStyle)).left().row();
		TextButton close = new TextButton("Close", skin);
		close.addListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {
